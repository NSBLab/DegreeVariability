function [CSTR,Clusters] = RunClusterPipelineProp(CSTR,Ncluster,makePlot,PROCESSING_MATRIX,PROCESSING_MATRIX_LABELS,ColorBarLabel)
...
n = size(CSTR,2)+1;
% [IDX,~,~,tree] = BF_ClusterReorder(CSTR, pdist(CSTR, 'chebychev'), 'single');
...
if makePlot
figure('Position',[0 0 2407 1887], 'DefaultAxesFontSize', 20);
...
for i=1:size(H,1)
    for j=round(H(i).YData(H(i).XData==0))
...
infoColors = [186,186,186; 64,64,64; 244,165,130; 171,217,233; 69,117,180; 49,54,149]/255;
...
colormap(ax1,infoColors(1:numel(unique(PROCESSING_MATRIX)), :));
